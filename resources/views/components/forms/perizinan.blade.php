<form action="{{ route('dashboard.' . roleName() . '.perizinan.update') }}" method="POST">
    @csrf
    <div class="row">
        @foreach ($fields as $field)
            <div class="mb-3 col-12 col-lg-6">
                ...
                @php
                    ...
                    $value = old($fieldName, $perizinan->{$fieldName} ?? '');
                ...
                @switch($field['type'])
                    ...
                    @case('select')
                        <select name="{{ $fieldName }}{{ $field['multiple'] ? '[]' : '' }}" id="{{ $fieldName }}"
                            class="form-control select2 {{ $isInvalid }}" {{ $required }} {{ $disabled }}
                    ...
                    @default
                        <input type="{{ $field['type'] }}" name="{{ $fieldName }}" id="{{ $fieldName }}"
                            value="{{ $value }}" class="form-control {{ $isInvalid }}" {{ $required }}
                            {{ $disabled }}>
                @endswitch
